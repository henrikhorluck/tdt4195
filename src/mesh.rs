...
impl Mesh {
    ...
    pub unsafe fn to_vao(&self) -> u32 {
        create_triangle_vao(&self.vertices, &self.colors, &self.indices, &self.normals)
    }
}
...
impl Terrain {
    pub fn load(path: &str) -> Mesh {
        ...
        let (models, _materials) =
            tobj::load_obj(path, true).expect("Failed to load terrain model");
...
impl Helicopter {
    pub fn load(path: &str) -> Self {
        ...
        let before = std::time::Instant::now();
        let (models, _materials) =
            tobj::load_obj(path, true).expect("Failed to load helicopter model");
        let after = std::time::Instant::now();
        println!(
            "Done in {:.3}ms.",
            after.duration_since(before).as_micros() as f32 / 1e3
